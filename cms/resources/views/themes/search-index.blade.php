                ...
                <div class="filter-form form-sort">
                    <div class="sort-group">
                        <label class="font-weight-700">Sort by</label>
                         <form method="get" id="form-sort" action="{{ Request::getRequestUri() }}">
                    ...
                    @endphp
                    <div class="view-control-group"><a class="btn-grid active" href="{{ getPageUrlByCode('SEARCH-INDEX') }}?{{ http_build_query(empty2blank($parameters)) }}">Grid </a><a class="btn-detail" href="{{ getPageUrlByCode('SEARCH-INDEX-DETAIL') }}?{{ http_build_query(empty2blank($parameters)) }}"> Detailed</a></div>
                </div>
            </div>
            <div class="container nl-container">
                <div class="content-grid-view">
                    <div class="row gutter-20">
                        @if(isset($count_project) && $count_project > 0)
                        <div class="col-md-12">There are {{ $count_project }} results @if(request('search') !== null) for the keyword "{{ request('search') }}" @endif</div>
                        ...
                        @endforeach
                        @else
                            <div class="col-md-12 text-center">No Results @if(request('search') !== null) for keywords "{{ request('search') }}" @endif</div>
                        @endif
                    </div>

                    @if(isset($projects) && count($projects) > 0)
                    <div class="pagination">{!! $projects->appends(['tenure' => request('tenure'), 'purpose' => request('purpose'), 'district' => request('district'), 'type' => request('type'), 'direction' => request('direction'), 'search' => request('search')])->links() !!}</div>
                    @endif
                </div>
            </div>
        </section>
...
<script>
    ...
    //         var link_replace = link+'&sort='+$(this).val();
    //         window.location = link_replace;
    //     }
    // });
    ...
    $(".sort-by").on('click', function(){
        ...
        var link_replace = link+'&sort='+sort_new;
        window.location = link_replace;
    });
</script>      
@endpush
